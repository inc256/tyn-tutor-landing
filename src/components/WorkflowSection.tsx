...
const WorkflowSection = () => (
  <section id="workflow" className="py-24 md:py-32 bg-gradient-soft">
    <div className="container">
      ...
      >
        <span className="text-sm font-semibold uppercase tracking-wider text-primary">Workflow</span>
        ...
        {steps.map((step, index) => {
          ...
          return (
            <motion.div
              ...
              className="relative rounded-3xl border border-border/60 bg-card p-7 shadow-soft hover:shadow-elevated transition-all duration-300"
            >
              ...
              <div className="space-y-3 text-sm leading-relaxed text-muted-foreground">
                <div>
                  <span className="font-semibold text-foreground">Input: </span>{step.input}
                </div>
                <div>
                  <span className="font-semibold text-foreground">Process: </span>{step.process}
                </div>
                <div>
                  <span className="font-semibold text-foreground">Output: </span>{step.output}
                </div>
              </div>

              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-1/2 right-[-42px] h-0.5 w-20 bg-primary/20" />
              )}
            </motion.div>
          );
        })}
